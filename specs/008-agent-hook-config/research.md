...
**决策**: 不持久化中间重试状态。重试通过 `tokio::spawn` 内存中异步执行，重启后丢失未完成的重试。仅最终结果写入 `hook_executions` 表。
...
**替代方案**: 在 `hook_executions` 表添加 `retry_state` 列 + cron 捡起未完成的重试 — 被拒绝，因复杂度与可靠性收益不匹配。
...
- Hook 为只读观察者，执行顺序不影响 Agent 状态，但影响审计日志的可追溯性
- 串行执行天然支持阻塞模式（失败即中止后续 Hook）
- 每个 Hook 执行时间短（默认超时 10s），串行带来的总延迟可控
- 与 spec FR-012 一致

**替代方案**: 同一触发点内 Hook 并行执行 `tokio::join!` — 被拒绝，因阻塞模式下需要协调中止逻辑，且并发 audit 写入顺序不可预测。
