...
const CATALOG: &[ProductionQuery] = &[
    ...
    // `create_or_upgrade_to_v4` 的 `reasoning_effort` 列探测（`pragma_table_info`
    // 是 SQLite 内置表值函数，没有可用的索引；行保持 inactive，只做登记）。
    ProductionQuery {
        id: "t012.llm_presets.reasoning_effort_probe",
        sql: "",
        owner_phase: "US4",
        ...
        join_columns: &[],
        requirements: &[QueryPlanRequirement {
            query_id: "t012.llm_presets.reasoning_effort_probe",
            owner_phase: "US4",
            activation_task: "T050",
            table: "pragma_table_info",
            expected_access: AccessExpectation::Search,
    ...
    // `create_or_upgrade_to_v4` 的 `deletable` 列探测（`pragma_table_info` 是
    // SQLite 内置表值函数，没有可用的索引；行保持 inactive，只做登记）。
    ProductionQuery {
        id: "t012.global_configs.deletable_probe",
        sql: "",
        owner_phase: "migrations",
        activation_task: "T012M",
        table: "pragma_table_info",
        ...
        join_columns: &[],
        requirements: &[QueryPlanRequirement {
            query_id: "t012.global_configs.deletable_probe",
            owner_phase: "migrations",
            activation_task: "T012M",
            table: "pragma_table_info",
            expected_access: AccessExpectation::Search,
    ...
    // `Store::ensure_global_configs_deletable` 的 pragma 探测：库已存在但缺少
    // `deletable` 列时补列（与同表的 `t012.global_configs.*` 行同属 US1/T019）。
    ProductionQuery {
        id: "t012.global_configs.deletable_column_probe",
        sql: "",
        owner_phase: "US1",
        activation_task: "T019",
        table: "pragma_table_info",
        ...
        join_columns: &[],
        requirements: &[QueryPlanRequirement {
            query_id: "t012.global_configs.deletable_column_probe",
            owner_phase: "US1",
            activation_task: "T019",
            table: "pragma_table_info",
            expected_access: AccessExpectation::Search,
    ...
    // v2/v3 库升级前读取旧版派生搜索 schema 的探测（`sqlite_schema` 是
    // SQLite 内置表，没有可用的索引，只能按名称过滤）。
    ProductionQuery {
        id: "t012.migrations.legacy_v4_search_schema",
        sql: "",
        owner_phase: "migrations",
        activation_task: "T012M",
        table: "sqlite_schema",
        ...
        join_columns: &[],
        requirements: &[QueryPlanRequirement {
            query_id: "t012.migrations.legacy_v4_search_schema",
            owner_phase: "migrations",
            activation_task: "T012M",
            table: "sqlite_schema",
            expected_access: AccessExpectation::Search,
    ...
    // Plugin 持久化台账的历史索引/触发器探测（同上，`sqlite_schema` 依赖内置表）。
    ProductionQuery {
        id: "t012.migrations.plugin_ledger_schema_indexes",
        sql: "",
        owner_phase: "migrations",
        activation_task: "T012M",
        table: "sqlite_schema",
        active: false,
        dialect: QueryDialect::Sqlite,
        filter_columns: &["tbl_name", "type", "sql"],
        join_columns: &[],
        requirements: &[QueryPlanRequirement {
            query_id: "t012.migrations.plugin_ledger_schema_indexes",
            owner_phase: "migrations",
            activation_task: "T012M",
            table: "sqlite_schema",
            expected_access: AccessExpectation::Search,
