...
**Primary Dependencies**: gpui/gpui-component、Tokio、SQLx `=0.9.0`（workspace 无共享 feature；HiveGUI 精确启用 `chrono|macros|runtime-tokio|sqlite`，HiveWeb 精确启用 `chrono|json|macros|mysql|runtime-tokio|rust_decimal|tls-rustls-ring-webpki`；两者的 `macros` 均已包含 derive，不重复声明 `derive`，HiveWeb 不启用 `mysql-rsa`）、HiveGUI 外部数据源用 `mysql_async =0.36.2` 的 `minimal|native-tls-tls`、`unicode-normalization = { version = "=0.1.25", default-features=false, features=["std"] }`（`UNICODE_VERSION=(17,0,0)`，只负责 Unicode 17 NFC；`NFKC_CF` 映射由官方 Unicode 17 数据生成）、`agent`、`providers`、`hive-builtins`、Extism `=1.30.0`（默认 feature 全关）与 Wasmtime `=43.0.2` `anyhow` 兼容 feature、wasmparser、reqwest、serde/serde_json、chacha20poly1305、sha2、tracing/tracing-appender；新增 `tokio-util` cancellation、`age` passphrase encryption、`tar` archive；Linux GUI 图通过 `zbus_xml =5.2.1` 与来自 `https://github.com/smithay/wayland-rs`、带来源证明的本地 `wayland-scanner =0.31.10`/`quick-xml =0.41` 最小兼容补丁；HiveWeb 使用 `aws-sdk-s3 =1.133.0` 且仅开启 `sigv4a|http-1x|default-https-client|rt-tokio`
...
**Scale/Scope**: 13 个 P1 用户故事、48 个功能需求、32 个成功标准；3 个顶层路由、12 个 AI 管理 Tab；20条/页；至少100节点 DAG、100+分类；本地单用户多并发会话
...
1. 先闭合独立质量基线 `codex/ci-quality-baseline-rust-1.97.1`：修复 `origin/main` 既有 fmt/Clippy/测试问题，删除已移除管理聊天能力的陈旧契约，并让现役 HiveWeb 集成测试在一次性 MySQL 8、Redis 7、MinIO 和迁移后的 CI 环境执行。该前置批次不实施 SQLx 0.9、严格 TLS、安全扫描、SQLx offline 或 HiveGUI 外部 DataSource。其后将 `codex/rust-1.97.1-toolchain` 重基到新基线，保留基础设施 job，并只以三文件差异精确固定 Rust 1.97.1、记录本地与 CI 的 `rustc --version` 一致性证据；原提交 `bf3690d` 已推送，但自动化 PR API 权限与远端 CI 仍 Pending，因此 T001 尚未完成。
...
- **架构边界**：在 `crates/hivegui/src/auth/` 下新增 `keystore.rs`（AuthKeystore 文件 IO + Argon2id 派生 + AES-256-GCM 包装/解包）、`lock.rs`（AuthLockState 内存态 + 空闲计时 + 屏幕锁事件 + 内存清零）、`policy.rs`（主密码强度规则、5 次错误 backoff、SQL 注入式扫除）、`ui/{setup,unlock,recovery_confirm}_view.rs`（GPUI 视图），与 `crates/hivegui/src/ui/app.rs`、`crates/hivegui/src/main.rs`、`crates/hivegui/src/runtime/mod.rs` 集成。
...
- **关键依赖**：新增 `argon2 =0.5`、`aes-gcm =0.10`（`aes` 走 `aes`+`alloc`，`aes-gcm` 关闭 `aes`/`alloc`/`default` 之外的 feature，与 FR-012 既有 `chacha20poly1305` 共存）；`zeroize =1.8` 用于内存清零；屏幕锁事件：`windows =0.58`（仅 `Win32_System_Power`，注册 `WM_WTSSESSION_CHANGE=0x7`）、`objc2 =0.5`（macOS `NSWorkspaceScreensDidSleep`）/ 复用 `zbus =4` `org.freedesktop.ScreenSaver ActiveChanged`。
...
- **风险声明**：本地主密码认证是 Constitution v1.5.0 Security Requirements 下“auth changes MUST receive dedicated security review + second approver” 范畴；按 2026-07-30 增补的 *Single-developer repository clause*，本仓库 1 名 active maintainer 同时承担 dedicated security review + second approver 角色，self-attestation 与 `/security-review` 结论须写入 `checklists/security.md` 对应行；FR-012 设备密钥、FR-049 主密码认证、FR-026 备份加密口令三者必须分别独立 security review，不允许一个签字覆盖另一个。
...
| HiveGUI 本地主密码认证（FR-049/FR-050/FR-051） | 桌面应用需要在 HiveWeb/Rustfs 不存在时保护本地敏感数据并实施用户级访问门禁。 | 接受"永远不锁定"或仅 OS 文件权限会失去对共享/被盗设备的保护；旁路重置/找回/安全问题会扩大攻击面。 | Argon2id (m=64MiB, t=3, p=1) 派生 wrapping KEK + AES-256-GCM 包装设备密钥材料；`keystore/` 0600 + 不进入任何备份；`zeroize` 内存清零；屏幕锁事件 fail-closed；5 次错误 backoff 5 分钟；无密码重置（仅可从 T129 备份恢复）。 | Cryptography/security reviewer 维护参数、OWASP 基线、KCV、备份 exclude 完整性；OWASP/RustCrypto/Argon2/AES-GCM 升级要求同步参数与派生耗时 < 5000ms 时钟验证。 |
