...
use agent::loop_::{BuiltinToolSet, ToolFactoryConfig, ToolFactoryDeps};
...
use providers::{GenerationSettings, GitHubCopilotProvider, LLMProvider, RetryMode, make_provider};
use session::SessionManager;
use dirs;

/// Expand `~` and `~user` in a path to the home directory.
...
/// Split a session key like `"channel:chat_id"` into `(channel, chat_id)`.
...
impl Nanobot {
    ...
    pub async fn from_config(
        ...
        let provider = make_provider(&cfg).map_err(NanobotError::Provider)?;
