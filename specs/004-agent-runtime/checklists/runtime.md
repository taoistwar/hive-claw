...
- [ ] CHK210 spec / research 是否明示 Pool 的**层级结构**（外层 `HashMap<PluginId, PluginPool>` + 内层 `VecDeque<Instance>`）？[Clarity, research §2]
...
- [ ] CHK216 128 MB linear memory 上限是否在 Plugin manifest 加载时强制（而非运行时 OOM 才发现）？[Clarity, FR-031 vs research §1]
- [ ] CHK217 调用栈深度上限是否定义？（防止 Plugin 通过递归触发栈溢出）[Coverage, Gap]
- [ ] CHK218 host_call payload 大小（4 MB 上下行）+ network.http body 大小（4 MB）是否在 spec 而非仅 contracts 中？[Consistency, host-functions.md §5]
- [ ] CHK219 多个 capability 的并发上限（如 `network.http` 8/Plugin）的共享池 vs 独立池语义是否在 spec 明示？[Clarity, host-functions.md §5]
...
- [ ] CHK221 fallback 之间的等待时间 / 退避策略是否定义？还是即刻切换？[Gap]
...
- [ ] CHK227 SC-006 (Agent routing ≤ 1.5s) 含的 1 次 LLM 决策调用是否假定 mock 固定响应时间？真实环境如何衡量？[Measurability, T156 mock 800ms]
- [ ] CHK228 SC-010 (端到端 ≤ 8s) 已登记为偏离 4（LLM 外部延迟）；spec 是否给出"宿主侧仅" 的可控延迟预算（如 ≤ 2s）作为内部目标？[Gap]
- [ ] CHK229 perf 测量在 hyper / hey / wrk 任一工具下都成立吗？工具选择是否在 tasks 显式（T140..T156）？[Consistency, tasks Polish]

## 并发与背压

- [ ] CHK230 SSE 长连接的并发上限（100 个 active session）是否在 spec 明示？超出时新连接行为？[Gap, plan §Scale]
- [ ] CHK231 Plugin 调用速率上限（per Agent / per session / 全局）是否定义？[Gap]
...
- [ ] CHK233 DB 连接池（sqlx pool）的 max_connections 在新增大量 audit 写入后是否需要重新调优？是否在 spec 给出建议？[Gap]
...
- [ ] CHK236 graceful shutdown 行为（SSE 长连接收尾 / Plugin 实例释放）是否在 spec 提及？[Gap]
- [ ] CHK237 在线 reload `llm_presets.toml` 的需求是否定义（避免重启服务）？还是 MVP 不支持？[Clarity, Gap]
...
- [ ] CHK239 Plugin 调用的 tracing span 字段（plugin_id / function_id / capability / outcome / elapsed_ms / pool_hit）是否完整定义？[Completeness, FR-021 v7]
- [ ] CHK240 LLM 调用的 tracing 字段（model_preset / actual_model / fallback_used / llm_ms）是否在 spec 明示？[Gap, research §7]
