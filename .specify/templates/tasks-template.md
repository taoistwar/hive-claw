...
**Tests**: Tests are REQUIRED by the constitution. For every production batch,
write tests first, obtain user/designated-reviewer approval, run them and record
the expected Red result, then implement the minimum Green change and refactor.
...
- **Web app**: `crates/` (Rust backend), `web/` or `frontend/` (TypeScript/React)
...
- [ ] T004 [P] Define versioned performance benchmark baselines, environment fingerprints, and the >10% regression comparison gate
...
> **NOTE: Write these tests FIRST, obtain approval, and record the expected Red
> result before any production implementation.**
...
- [ ] TXXX Re-run tracked benchmarks and block any unapproved >10% regression
...
- Tests MUST be written, approved, and observed to FAIL before implementation
...
```bash
# Launch all approved Red tests for User Story 1 together:
