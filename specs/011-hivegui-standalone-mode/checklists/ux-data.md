...
- [x] CHK021 `master_password` 表仅存储单条记录，是否需要定义多记录场景的处理规则（如 SQLite 文件被手动篡改插入多条）？[Edge Case, Data Model §master_password]
  > **评估**: 应用层保证单记录（id=1）。多记录场景仅在 SQLite 文件被外部工具篡改时发生。FR-014 的 integrity_check 可检测异常，但不在 spec 中定义每条篡改场景的处理。属于防御性编程的实现细节。
...
- [x] CHK024 `workflow_nodes.node_type` 的 CHECK 约束包含 'start', 'end', 'function', 'generate_answer'——是否需要在需求层面明确每个工作流必须有且仅有一个 start 和一个 end？[Completeness, Data Model §workflow_nodes, FR-003]
  > **评估**: DAG 执行器要求唯一 start 节点（拓扑排序入口）。此约束在 T053 WorkflowExecutor 中验证（执行前检查合法性），而非数据库 CHECK 约束。data-model 的 CHECK 约束提供基础类型验证，业务约束由运行时执行。
...
- [x] CHK027 `function_registry.function_type` 的 plugin 类型需要 `plugin_id` 外键——是否需要在需求中明确插件删除时的级联行为（插件删除后关联函数是否一并删除）？[Gap, Data Model §function_registry]
  > **评估**: T064 集成插件函数到 FunctionRegistry——插件注册后自动添加函数。插件删除时（T061 `delete_plugin()`）级联删除关联的 function_registry 记录。data-model §function_registry 的 plugin_id 为可选字段（仅 plugin 类型需要）。
...
- [x] CHK029 SQLite 不支持 ENUM，使用了 CHECK 约束替代（如 `function_type TEXT CHECK (function_type IN ('builtin', 'custom', 'plugin'))`）。这些 CHECK 约束是否在所有相关表中完整定义并无遗漏？[Completeness, Data Model §Table Definitions]
  > **评估**: Data-model.md 在所有枚举字段上定义了 CHECK：function_type, node_type, match_type, action, record_type, status。唯一隐式枚举 agent_configs.is_active/plugins.is_active/sensitive_word_rules.is_active 为 INTEGER 0/1。覆盖充分。
...
- CHK001–CHK040 全部通过评估
- 在备注中记录发现的具体问题或改进建议
- CHK001–CHK040 顺序编号，引用 spec.md 章节和数据模型表名作为追溯
