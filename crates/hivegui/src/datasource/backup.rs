...
impl BackupExporter {
    ...
    pub async fn export_age(
        ...
        let prepared = prepare_canonical_export(&source, None).await?;
...
async fn prepare_canonical_export(
    ...
    let anchored_source = open_export_source_anchored(source)?;
    ...
    if checkpoint.0 != 0 {
        pool.close().await;
        return Err(ExportError::Io("checkpoint busy".into()));
    }
    ...
    let cleanup_result = async {
        ...
        export_entity_files(&snapshot_pool, &source_crypto).await
    }
    .await;
    let entity_files = match cleanup_result {
        Ok(entity_files) => entity_files,
        Err(error) => {
...
fn decrypt_age_streaming(archive: &Path, passphrase: &str) -> Result<Box<dyn Read>, ImportError> {
    let file = open_verified_archive(archive)?;
...
fn open_verified_archive(archive: &Path) -> Result<File, ImportError> {
    let parent = archive
        .parent()
    ...
    let name = archive
        .file_name()
        .ok_or_else(|| ImportError::UnsafeArchiveEntry(archive.display().to_string()))?;
    let parent = open_ambient_directory_nofollow(parent)?;
    let entry = parent
...
fn decompress_and_parse(reader: Box<dyn Read>) -> Result<ParsedArchive, ImportError> {
...
fn import_blocking(
    ...
    let reader = decrypt_age_streaming(archive, passphrase)?;
    let parsed = decompress_and_parse(reader)?;
...
fn inspect_manifest_blocking(
    ...
    let reader = decrypt_age_streaming(archive, passphrase)?;
    Ok(decompress_and_parse(reader)?.manifest)
}
...
impl BackupCoordinator {
    ...
    async fn confirm_export_internal(
        ...
        let names = sqlx::query_scalar::<_, String>("SELECT name FROM data_sources ORDER BY name")
            ...
            .await
            .map_err(|_| ExportError::Io("confirmed inventory failed".into()))?;
        let source = self.store.database_path().to_path_buf();
        let prepared = prepare_canonical_export(&source, crash_at).await?;
...
pub struct RestoreCoordinator {
    root: PathBuf,
    root_directory: std::sync::Arc<cap_std::fs::Dir>,
}
...
impl RestoreCoordinator {
    ...
    pub fn new(root: impl Into<PathBuf>) -> Result<Self, ImportError> {
        ...
        let coordinator = Self {
            root,
            root_directory: std::sync::Arc::new(root_directory),
        };
    ...
    pub async fn prepare_restore(
        ...
        let import_result = async {
            let manifest = BackupImporter::new(&staging_directory)
                .inspect_manifest(archive, passphrase)
                .await?;
            BackupImporter::new(&staging_directory)
                .import_age_with_key_root(archive, passphrase, staging_directory.clone(), io_root)
                .await?;
    ...
    pub async fn apply_restore(&self, plan: &RestorePlan) -> Result<RestoreRecovery, ImportError> {
        self.apply_restore_internal(plan, None).await
    }
    ...
    async fn apply_restore_internal(
        ...
        let safety_snapshot = create_restore_safety_snapshot(
            &io_root,
            &current_database,
            &current_plugins,
            &plan.db_instance_operation_id,
        )?;
        verify_restore_safety_snapshot(&safety_snapshot)?;
        ...
        let mut owner = RestoreOwner {
            ...
            old_database: optional_controlled_file_evidence(
                &current_database,
                Path::new(DATABASE_FILENAME),
            )?,
            ...
            safety_snapshot: controlled_tree_evidence(
                &safety_snapshot,
        ...
        if current_database.exists() {
            publish_no_replace(&current_database, &old_database)
                .map_err(|error| ImportError::Io(error.to_string()))?;
        }
        if let Err(error) = publish_no_replace(&staging_database, &current_database) {
    ...
    pub async fn apply_with_crash(
        ...
        match self.apply_restore_internal(plan, Some(crash_point)).await {
    ...
    pub async fn recover_startup(&self) -> Result<RestoreRecovery, ImportError> {
        ...
        frozen_databases()
            .lock()
            .map_err(|_| ImportError::Io("write gate poisoned".into()))?
            .remove(&self.root.join(DATABASE_FILENAME));
        Ok(RestoreRecovery {
            retirement_outcome,
            store_may_open: true,
...
fn copy_plugin_tree_with_descriptors(
    ...
    fs::create_dir(target).map_err(|error| ImportError::Io(error.to_string()))?;
    ...
    for entry in entries {
        ...
        if metadata.is_dir() {
            copy_plugin_tree_with_descriptors(&source_path, &target_path, &relative_path, out)?;
        } else if metadata.is_file() {
            out.push(copy_owned_regular_file(
                &source_path,
                &target_path,
                &relative_path,
            )?);
...
fn create_restore_safety_snapshot(
    ...
    let snapshots = root.join("backups");
    fs::create_dir_all(&snapshots).map_err(|error| ImportError::Io(error.to_string()))?;
    let snapshot = snapshots.join(format!("restore-safety-{operation_id}"));
    fs::create_dir(&snapshot).map_err(|error| ImportError::Io(error.to_string()))?;
    let database = if current_database.exists() {
        Some(copy_owned_regular_file(
            current_database,
            &snapshot.join("datasources.db"),
            Path::new("datasources.db"),
        )?)
    } else {
        None
    };
    ...
    let plugin_root = if current_plugins.exists() {
        ...
        let copied = controlled_tree_evidence(&snapshot.join("plugins"), Path::new("plugins"))?;
    ...
    write_staging_file(&manifest_path, &bytes)?;
    sync_tree_bottom_up(&snapshot)?;
    sync_directory(&snapshots)?;
    Ok(snapshot)
}

fn verify_restore_safety_snapshot(snapshot: &Path) -> Result<(), ImportError> {
    ...
    let database_path = snapshot.join("datasources.db");
    match manifest.database.as_ref() {
        Some(database) => {
            if database.path != "datasources.db"
                || database.source_identity.is_empty()
                || inspect_owned_regular_file(&database_path)?.is_none_or(|evidence| {
    ...
    for artifact in &manifest.plugin_files {
        ...
        if previous.as_ref().is_some_and(|path| path >= &artifact.path)
            ...
            || inspect_owned_regular_file(&snapshot.join(&relative))?.is_none_or(|evidence| {
    ...
    if collect_relative_regular_files(&snapshot.join("plugins"))? != expected_plugins {
        return Err(ImportError::InvalidManifest(
            "safety Plugin inventory mismatch".into(),
        ));
    }
    let snapshot_plugins = snapshot.join("plugins");
    match manifest.plugin_root.as_ref() {
        Some(plugin_root) => {
            if plugin_root.path != "plugins"
                || plugin_root.source_identity.is_empty()
                || controlled_tree_evidence(&snapshot_plugins, Path::new("plugins"))?
        ...
        None if snapshot_plugins.exists() || !manifest.plugin_files.is_empty() => {
            return Err(ImportError::InvalidManifest(
...
fn restore_missing_old_from_safety_snapshot(
    ...
    verify_restore_safety_snapshot(&snapshot)?;
