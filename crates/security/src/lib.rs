...
fn parse_url(url: &str) -> Option<(String, String, String)> {
    ...
    // Strip userinfo (`user:pass@`), then host[:port].
...
fn resolve_host(hostname: &str) -> Result<Vec<IpAddr>, std::io::Error> {
    // Port 0 just to satisfy ToSocketAddrs; DNS doesn't care about the port.
...
// Re-export for fmt compatibility.
#[allow(dead_code)]
fn _ip_v4_example() -> Ipv4Addr {
    Ipv4Addr::new(127, 0, 0, 1)
}
#[allow(dead_code)]
fn _ip_v6_example() -> Ipv6Addr {
    Ipv6Addr::LOCALHOST
}
