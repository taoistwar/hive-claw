...
const schemaStyle: React.CSSProperties = {
  fontSize: 12,
  margin: 0,
  maxHeight: 200,
  overflow: 'auto',
  ...
  return (
    <div>
      ...
      <Card title="权限要求" size="small" style={{ marginBottom: 16 }}>
        <Descriptions bordered column={1} size="small">
          <Descriptions.Item label="required_capabilities">
            {fn.required_capabilities && fn.required_capabilities.length > 0 ? (
              fn.required_capabilities.map((c) => <Tag key={c}>{c}</Tag>)
            ) : (
              '—'
            )}
          </Descriptions.Item>
